...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Control an active Pryva tool-backed voice run. Use this when the caller asks in any language for status/progress, cancellation, a redirect/change to the active work, or a follow-up after the current work. Do not use this for ordinary greetings or chatter unless the caller is asking about the active work.",
  parameters: {
    type: "object",
    properties: {
      text: {
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal Pryva voice control result.",
    "Do not call openclaw_agent_consult or any other tool for this message.",
    "Speak this exact Pryva status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "Pryva is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "Pryva has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "Pryva could not queue that follow-up."
    : "Pryva could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "Pryva finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `Pryva is starting ${name}.` : "Pryva is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `Pryva finished ${name} and is continuing.`
        : "Pryva finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `Pryva is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "Pryva is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `Pryva is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "Pryva is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "Pryva is working on the current voice request.";
  }
