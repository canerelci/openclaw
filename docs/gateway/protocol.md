...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - Pryva self-turn params (accepted by both `sessions.send` and `sessions.steer`, which share one handler): `innerVoice: true` runs `message` as the assistant's own framed thought (a cron-backed self-turn) instead of ingesting it as an inbound; `pryvaFlowId` / `pryvaFlowSource` attribute the turn to an existing flow; `delaySeconds` schedules the wake; `mustSpeak` drops the "nothing worth saying → NO_REPLY" escape for a notification the owner is waiting on; and `pryvaSilent: true` marks the turn as background work, forcing `deliveryMode: "none"` on the scheduled cron job. A silent turn is *structurally* undeliverable rather than merely discouraged: with mode `none` and no explicit cron delivery target the runner reports `deliveryRequested: false` and never resolves a delivery target, so the `"last"`-channel fallback — which otherwise picks the single configured channel and can take the recipient from `allowFrom[0]` — is never reached. The turn still runs its tools and logs its flow steps normally, and flow attribution is unchanged. All of these are ignored unless `innerVoice` is true.
